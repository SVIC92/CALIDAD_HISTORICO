...
const HorarioProfesor = () => {
  ...
  useEffect(() => {
    if (!selectedProfesor) { setHorarios([]); return; }
  ...
  return (
    <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', height: '100%', color: 'text.primary' }}>
      ...
      >
        ...
        <TextField
          select fullWidth label="Seleccionar Profesor"
      ...
      >
        ...
        <DialogContent dividers>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mb: 2, gap: 0.5 }}>
            ...
            {claseActiva?.modalidad && (
              <Chip label={claseActiva.modalidad} color="secondary" size="small" variant="outlined" />
            )}
            {curso?.estado && (
        ...
        <DialogActions>
          <Button onClick={() => setClaseActiva(null)} variant="contained" color="secondary" size="small">
            Cerrar
          </Button>
        </DialogActions>
      </Dialog>
