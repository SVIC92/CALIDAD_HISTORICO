...
const CursoService = {
  ...
  registro: async (payloadOrNombre, descripcion) => {
    ...
    const profesorAsignado =
      payload.profesorAsignado?.trim?.() || payload.profesorAsignado;
  ...
  agregarHorario: async (cursoId, horarioData) => {
    const respuesta = await axios.post(
      `/curso/${cursoId}/horarios/agregar`,
      null,
      {
        params: horarioData,
      },
  ...
  listarHorariosProfesor: async (profesorId) => {
    const respuesta = await axios.get(`/curso/horarios/profesor/${profesorId}`);
    return respuesta.data;
  },
  listarMisHorarios: async () => {
    const respuesta = await axios.get("/curso/horarios/alumno");
    return respuesta.data;
  },
};
